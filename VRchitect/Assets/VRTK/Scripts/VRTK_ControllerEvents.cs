...
namespace VRTK
{
    ...
    public class VRTK_ControllerEvents : MonoBehaviour
    {
        ...
        {
			/*GameObject rightController = GameObject.Find ("Controller (right)");
			if (this.transform.parent = rightController.transform) {
				rightController.GetComponentInChildren<test> ().TurnOnOffMenu (rightController);
			}*/
            GameObject GameController = GameObject.Find("GameController");
			GameObject camera = GameObject.Find ("Camera (eye)");
            if (GameObject.Find("GameController").GetComponent<GameStateControl>().State == GameStateControl.GameState.DELETE)
                GameObject.Find("GameController").GetComponent<GameStateControl>().State = GameStateControl.GameState.IN_ROOM;
			camera.GetComponentInChildren<MainMenuScript> ().TurnOnOffMainMenu ();
            GameController.GetComponent<GameStateControl>().DisableAllTips();
            if (GameController.GetComponent<GameStateControl>().showingtips) { 
                 if (GameController.GetComponent<GameStateControl>().showingCubeMenu) 
                     GameController.GetComponent<GameStateControl>().TipCanvas_CubeMenuInstruction.gameObject.SetActive(true);
                 else
                 {
